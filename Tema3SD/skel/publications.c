...
PublData* init_publ_data(void) {
  ...
  data->ht_aux=malloc(sizeof(struct Hashtable));
   ...
   data->ht_influenced=malloc(sizeof(struct Hashtable));
    ...
    data->ht_fields->buckets = (struct LinkedList *) malloc(NMAX * sizeof(struct LinkedList));
   
    data->ht_fields->size = 0;
    data->ht_fields->hmax = NMAX;
    data->ht_fields->hash_function = hash_function_int;
   data->ht_fields->compare_function = compare_function_ints;
  for (i = 0; i < NMAX; i++) {
      ...
      data->ht_venue->buckets[i].size = 0;
       data->ht_fields->buckets[i].head = NULL;
      data->ht_fields->buckets[i].size = 0;
    }
...
void put_venue(struct Hashtable *ht, char* venue,const int64_t id) {
 ...
 //struct  influenced *new=malloc(sizeof(struct influenced));
   new->venue = (char *) malloc((strlen(venue)+1) * sizeof(char));
  snprintf(new->venue, strlen(venue)+1, "%s", venue);
...
int compare_function(struct Node* node1,struct Node* node2){
struct field_aux* value1=(struct field_aux*)node1->data;
struct field_aux* value2=(struct field_aux*)node2->data;

//printf("%lld %lld\n",value1->id,value2->id);
  if(value1->cit<value2->cit)
  {
    return 1;
  }
  if(value1->cit>value2->cit)
...
void put_field(struct Hashtable *ht,struct Hashtable* ht1, char* field,const int64_t id,const int year,char *nume) {
  //printf("%lld %lld ",id,year);
    /* TODO */
    struct Node *curr;
   struct Node *curr1,*curr2,*curr3;
   unsigned long id_aux =id;
   struct LinkedList* list;
   int64_t id_tmp=id;
   int ind=hash_function_string(field) % ht->hmax;
    curr=ht->buckets[ind].head;
     curr1=curr;
     //printf("%s %lld %lld %s->finish<-\n",nume,id_tmp,year,field);

     struct field_aux* aux=malloc(sizeof(struct field_aux));
     aux->id=id_tmp;
     aux->year=year;
  //   printf("%d\n",(int*)get_aux(ht1,id_tmp));
     if(get_aux(ht1,id_tmp)!=NULL){
     aux->cit=(int*)get_aux(ht1,id_tmp);}else{
      aux->cit=1;
     }
//printf("enter--%d\n",aux->cit);
    aux->nume = (char *) malloc((strlen(nume)+1) * sizeof(char));
  snprintf(aux->nume, strlen(nume)+1, "%s", nume);
 // printf("%s ",field);
  //printf("%lld ",id_tmp);
  //printf("%d", );
  struct Node* new_node=malloc(sizeof( struct Node));
  ...
  while(curr!=NULL){
    ...
    if(strcmp(((struct field *)curr->data)->field,field)==0){

      list=((struct field *)curr->data)->list;
      curr2=list->head;
      curr3=curr2;
      while(curr2!=NULL){
        //printf("%lld %lld\n",((struct field_aux*)curr2->data)->id,aux->id);
              //printf("sex\n");
       if(compare_function(new_node,curr2)==0){
        break;         


        }
        curr3=curr2;
        if(compare_function(new_node,curr2)==1){
          //printf("sex2\n");
        curr2=curr2->next;         
...
//printf("sex3\n");
  struct  field *new=malloc(sizeof(struct field));
   new->field = (char *) malloc((strlen(field)+1) * sizeof(char));
  snprintf(new->field, strlen(field)+1, "%s", field);
  new->list=malloc(sizeof(struct LinkedList));
  new->list->head=malloc(sizeof( struct Node));
  new->list->head->data=aux;
  new->list->head->next=NULL;
  new->list->size=1;
  ht->size++;


struct Node* new_node_aux=malloc(sizeof(struct Node));
new_node_aux->data=new;
new_node_aux->next=NULL;
if(curr1==NULL){
  ht->buckets[ind].head=new_node_aux;
  ht->buckets[ind].size++;
}else{
curr1->next=new_node_aux;
  ht->buckets[ind].size++;

}

        ht->buckets[ind].tail = new_node_aux;
       // printf("sex1\n");
  //add_nth_node(&ht->buckets[ind],ht->buckets[ind].size,new);
...
struct field* get_field(struct Hashtable *ht, char* field) {
    /* TODO */
 // printf("%d\n",key);
    struct Node *curr;
    // unsigned long int h=key;
     
  int hash_index = hash_function_string(field) % ht->hmax;
...
void new_free_ht_venue(struct Hashtable *ht) {
  int i;
  struct LinkedList list;
  struct LinkedList* heh;
  struct Node *curr1,*curr, *prev,*prev1;
 
  struct venue *prev_info;
  for(i = 0; i < ht->hmax; i++) {
    list = ht->buckets[i];
    curr = list.head;
    if(curr != NULL) {
      ...
      while (curr != NULL) {
        heh= ((struct venue *)curr->data)->list;
        curr1=heh->head;
        while(curr1!=NULL){
          prev1=curr1;
          curr1=curr1->next;
        ...
        free(((struct venue *)curr->data)->venue);
        prev = curr;
        curr = curr->next;
        prev_info = (struct influenced*)(prev->data);
        //free_paper(prev_info->value);
...
void destroy_publ_data(PublData* data) {
  ...
  new_free_ht_venue(data->ht_venue);
  free(data);
}
...
int64_t freq=id;
  ...
  for (i = 0; i < num_fields; i++) {
   ...
   // printf("%s %lld %d %s----",my_paper->title,freq,my_paper->year,my_paper->fields[i]);

    put_field(data->ht_fields,data->ht_aux,my_paper->fields[i],freq,my_paper->year,my_paper->title);
   //printf("ss\n");
  }
  char* s=venue;
...
//}
  ...
  //printf("%s\n",my_paper->title);
  //int g=get_aux(data->ht_aux,freq);
  //if(g!=NULL){
 // printf("%s %lld %d %d\n",my_paper->title,my_paper->id,my_paper->year,g);
//}
}
...
char** get_most_cited_papers_by_field(PublData* data, const char* field,
 ...
 // printf("cacat\n");
  ...
  int i;
  struct Node* curr;
  struct LinkedList* list=get_field(data->ht_fields,field)->list;
if(list->size<*num_papers){
  *num_papers=list->size;
}
matrix=malloc((*num_papers)*sizeof(char));
curr=list->head;
 i=0;
while(curr!=NULL&&i<*num_papers){
  //printf("%s\n",(char*)((struct field_aux *)curr->data)->nume);
   matrix[i] = (char *) malloc((strlen((char*)((struct field_aux *)curr->data)->nume)+1) * sizeof(char));
  snprintf(matrix[i], strlen(((struct field_aux *)curr->data)->nume)+1, "%s", ((struct field_aux *)curr->data)->nume);
  curr=curr->next;
  i++;
